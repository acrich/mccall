...
w_draws = lognormal_draws(n=2000, μ=0.5, σ=1)
...
def update_v(v, h, d):
    ...
    for i in range(a_size):
        for j in range(w_size):
            ...
            rhs = u(consumption, a_grid) + β*((1 - α)*v[:, j] + α*d*0.1)
...
def update_h(v, h, d, a_opt_employed):
    ...
    for i, a in enumerate(a_grid):
        ...
        rhs = u(consumption, a_grid) + β*d*0.1
        rhs_opt = np.nanmax(rhs)
...
u_t, a, consumption, realized_wage, employment_spells, separations = draw_lifetime(T, a_0_value=1)
print(np.mean(realized_wage*employment_spells))
u_t, a, consumption, realized_wage, employment_spells, separations = draw_lifetime(T, a_0_value=50)
print(np.mean(realized_wage*employment_spells))
a = a[:-1].copy()
